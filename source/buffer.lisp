...
(export-always 'buffer-local-histories-alist)
(defmethod buffer-local-histories-alist ((history htree:history-tree))
  "Returns an alist mapping buffer `id''s to the histories of the corresponding buffers."
  (let ((buffers (make-hash-table :test #'string-equal)))
    (loop for node in (htree:all-nodes history)
          do (setf (gethash (id node) buffers)
                   (cons node (gethash (id node) buffers)))
             ;; TODO: Maybe return hash-table itself, instead of alist?
          finally (return (alex:hash-table-alist buffers)))))

(export-always 'buffer-local-history-tree)
(defmethod buffer-local-history-tree ((buffer buffer))
  "Returns a node that the buffer-local history starts from
and all the nodes of buffer-local history."
  (loop with id = (id buffer)
        with buffer-history
          = (assoc id (buffer-local-histories-alist
